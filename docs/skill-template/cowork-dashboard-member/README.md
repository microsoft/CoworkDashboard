...
Grab the whole skill as one file: [`cowork-dashboard-member.zip`](../cowork-dashboard-member.zip) — the generic,
...
- **Team channel (asked on first run):** the skill is **not** tied to any one channel. On a member's
  ...
  (`/mnt/user-config/.claude/cowork-dashboard-member-channel.<userkey>.json`) for later runs. Your team's
...
> the **`cowork-roi-report-aggregated`** table post and the manager-side **`cowork-dashboard-team-dashboard`**
