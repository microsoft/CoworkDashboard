...
TABLE-FORMATTED Teams channel message (HTML body) for the team Cowork Dashboard rollup.
...
def build_message(d):
    ...
    H.append(f"<p><b>📊 Cowork Dashboard — team stats</b><br>Period: {esc(win['label'])} "
    ...
    #    Every deliverable is made visible (a de-identified DESCRIPTIVE label +
    #    type + date), labelled with the business process it supported. The
    #    Deliverable column shows a clean descriptive name with person names,
    #    customer/account names, and raw file names removed — NEVER a raw file name.
    if delivs:
        H.append(hdr("📦 Deliverables &amp; the skills behind them"))
        per_rows = []
        for dv in sorted(delivs, key=lambda x: -x.get("value", 0)):
            per_rows.append((dv.get("name", "—"), dv.get("type", "File"), dv.get("date", ""),
                             dv.get("process", "—"),
                             ", ".join(s for s in dv.get("skills", []) if s) or "—",
                             f"{dv.get('hours', 0)} h", money(dv.get("value", 0))))
        H.append(table(["Deliverable", "Type", "Date", "Business process", "Skills", "Hours", "Value"], per_rows))
    ...
    H.append('<p style="font-size:12px;color:#605E5C">Methodology: research-anchored per-category '
             ...
             'directional (expert ÷ modeled assisted clock). De-identified — person names, raw file '
             'names, prompts and country excluded (deliverables shown as de-identified descriptive '
             'labels); the runner\'s directory role is shown. '
             'Generated by the Cowork Dashboard member skill.</p>')
    return "".join(H)
