...
_PROCESS_GROUPS_PATH = os.path.join(_DIR, "process_groups.json")
...
# ---------------------------------------------------------------------------
# Canonical business-process GROUPING — collapse the long tail of process labels
# (APQC fallback labels, per-user registry-minted names, overrides) into ONE
# short, stable set so the "Work by business process" table isn't a long list.
# Only the process LABEL is grouped; pillars and Jobs-to-be-done stay granular.
# SHARED CONTRACT: process_groups.json is duplicated in the member + report
# bundles and read (via the report copy) by the aggregated "Dashboard" skill.
# ---------------------------------------------------------------------------

def load_process_groups(path: str = _PROCESS_GROUPS_PATH):
    """Load process_groups.json. Returns None (=> no grouping / identity) if the
    file is missing, so the classifier still runs on an older bundle."""
    try:
        with open(path) as f:
            d = json.load(f)
    except Exception:
    ...
    return {
        ...
        "apqc": {str(k).lower(): v for k, v in (d.get("apqc_to_group") or {}).items()},
        "rules": d.get("keyword_rules") or [],
        "default": d.get("default", ""),
    }


def group_process(label: str, groups) -> str:
    """Map any process label to its canonical group. Order of precedence:
    already-canonical group -> exact APQC label -> first ordered keyword rule
    whose keyword is a substring -> keep the original name (novel minted work is
    preserved; fold it into a group later by renaming it in the registry)."""
    if not groups or not label:
        return label
    lab = str(label).strip()
    low = lab.lower()
    if lab in groups["groups"]:
        return lab
    if low in groups["apqc"]:
        return groups["apqc"][low]
    for rule in groups["rules"]:
        for kw in rule.get("any", []):
            if kw and kw.lower() in low:
                return rule.get("group", lab)
    return lab
...
def main(inp: str, out: str, overrides_path: str = _OVERRIDES_PATH) -> None:
    ...
    proc_groups = load_process_groups()
    ...
    for s in sessions:
        ...
        # Collapse the resolved label into its canonical business-process group
        # (after pillar/job were resolved from the original label, so those stay
        # accurate). Keeps the process table a short, stable list.
        process = group_process(process, proc_groups)
